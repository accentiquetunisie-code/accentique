      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 mb-16">
        ...
        <div className="space-y-4">
          <div className="relative">
            <img src={getCurrentImage()} alt={product.name} width={800} height={500} fetchPriority="high" decoding="async" className="w-full h-96 lg:h-[500px] object-cover rounded-lg shadow-lg" />
          ...
          {product.images.length > 1 && !selectedVariant && <div className="flex space-x-2">
            {product.images.map((image, index) => <button key={index} onClick={() => setSelectedImageIndex(index)} className={`w-20 h-20 rounded-lg overflow-hidden border-2 transition-colors ${index === selectedImageIndex ? 'border-gold-500' : 'border-gray-200'}`}>
              <img src={image} alt={`${product.name} ${index + 1}`} loading="lazy" decoding="async" width={80} height={80} className="w-full h-full object-cover" />
            </button>)}
          </div>}
        </div>
